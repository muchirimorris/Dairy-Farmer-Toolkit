...
class _AnalyticsScreenState extends State<AnalyticsScreen> {
  ...
  void initState() {
    ...
    if (user != null) {
      ...
      _milkStream = _milkLogRepo.getMilkLogsStream(user.uid).asBroadcastStream();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return MainLayout(
      selectedIndex: 2, // Reusing existing index for layout, though it's technically separate
      child: Scaffold(
        appBar: AppBar(
          title: const Text("📊 Farm Analytics", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
      ...
      builder: (context, animalSnapshot) {
        return StreamBuilder<List<MilkLogModel>>(
          ...
          builder: (context, milkSnapshot) {
            ...
            for (var log in logs) {
              yields[log.animalId] = (yields[log.animalId] ?? 0.0) + log.quantity;
            }
            ...
            final bestAnimal = animals.firstWhere((a) => a.id == bestAnimalId, orElse: () => animals.first);

            return Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text("🏆 Top Producer", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                ...
                Card(
                  ...
                  child: ListTile(
                    ...
                    title: Text(bestAnimal.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 20)),
                    subtitle: Text("${topAnimals.first.value.toStringAsFixed(1)} L Total Yield"),
                  ),
                ),
                const SizedBox(height: 16),
                const Text("Top 3 Animals", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                ...topAnimals.asMap().entries.map((entry) {
                  final index = entry.key;
                  final animalId = entry.value.key;
                  final amount = entry.value.value;
                  final animalName = animals.firstWhere((a) => a.id == animalId, orElse: () => animals.first).name;
    ...
    return StreamBuilder<List<AnimalModel>>(
      ...
      builder: (context, snapshot) {
        ...
        for (var animal in animals) {
          statusCount[animal.productionStatus] = (statusCount[animal.productionStatus] ?? 0) + 1;
        }

        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text("🐄 Herd Distribution", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            Card(
              elevation: 2,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
              child: Padding(
                ...
                child: Column(
                  children: statusCount.entries.map((entry) {
                    final percent = animals.isNotEmpty ? entry.value / animals.length : 0.0;
                    return Padding(
                      padding: const EdgeInsets.symmetric(vertical: 8),
                      child: Row(
                        children: [
                          SizedBox(width: 80, child: Text(entry.key, style: const TextStyle(fontWeight: FontWeight.bold))),
