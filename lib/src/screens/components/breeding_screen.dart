...
class _BreedingScreenState extends State<BreedingScreen> {
  ...
  Map<String, dynamic> _calculatePredictions(String eventType, DateTime eventDate) {
  ...
  Future<void> _saveBreedingRecord() async {
    ...
    if (_formKey.currentState!.validate() && 
      ...
      final predictions = _calculatePredictions(_selectedEventType!, _selectedDate!);
  ...
  Future<void> _deleteRecord(String docId, String animalName) async {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("Are you sure you want to delete this breeding record for $animalName?"),
        actions: [
          TextButton(
  ...
  Widget _buildUpcomingReminders(QuerySnapshot snapshot) {
    ...
    for (final doc in snapshot.docs) {
      ...
      if (record.containsKey('earlyPregnancyCheck')) {
        final earlyCheckDate = (record['earlyPregnancyCheck'] as Timestamp).toDate();
          ...
          children: [
            ...
            ...upcomingRecords.map((doc) {
              ...
              if (record.containsKey('predictedCalving')) {
                ...
                if (daysUntilCalving >= 0 && daysUntilCalving <= 30) {
                  String urgency = daysUntilCalving <= 7 ? "🚨 URGENT" : "📅 Upcoming";
                  reminders.add(_buildReminderCard(
                    icon: Icons.pregnant_woman,
                    color: daysUntilCalving <= 7 ? Colors.red : Colors.green,
                    title: "$urgency: Expected Calving",
                    subtitle: "$animalName - ${DateFormat('MMM dd, yyyy').format(calvingDate)} (in $daysUntilCalving days)",
                    type: "Calving",
                  ));
                }
              }
              ...
              if (record.containsKey('predictedNextHeat')) {
                ...
                if (daysUntilHeat >= 0 && daysUntilHeat <= 7) {
                  reminders.add(_buildReminderCard(
                    ...
                    subtitle: "$animalName - ${DateFormat('MMM dd, yyyy').format(heatDate)} (in $daysUntilHeat days)",
                    type: "Heat",
                  ));
                }
              }
              ...
              if (record.containsKey('pregnancyCheckDue')) {
                ...
                if (daysUntilCheck >= 0 && daysUntilCheck <= 14) {
                  reminders.add(_buildReminderCard(
                    ...
                    subtitle: "$animalName - ${DateFormat('MMM dd, yyyy').format(checkDate)} (in $daysUntilCheck days)",
                    type: "Check",
                  ));
                }
              }
              ...
              if (record.containsKey('earlyPregnancyCheck')) {
                ...
                if (daysUntilEarlyCheck >= 0 && daysUntilEarlyCheck <= 7) {
                  reminders.add(_buildReminderCard(
                    ...
                    subtitle: "$animalName - ${DateFormat('MMM dd, yyyy').format(earlyCheckDate)} (in $daysUntilEarlyCheck days)",
                    type: "Early Check",
                  ));
                }
              }
              ...
              return Column(
                children: [
                  ...reminders,
                  if (upcomingRecords.indexOf(doc) != upcomingRecords.length - 1 && reminders.isNotEmpty)
                    const SizedBox(height: 8),
                ],
              );
            }).toList(),
  ...
  Widget _buildAnalyticsSection(QuerySnapshot snapshot) {
    ...
    final heatEvents = records.where((r) => r['eventType'] == 'Heat Detection').length;
    final aiEvents = records.where((r) => r['eventType'] == 'Insemination (AI)').length;
    final successfulPregnancies = records.where((r) => r['outcome'] == 'Pregnancy Confirmed').length;
    final calvingEvents = records.where((r) => r['eventType'] == 'Calving').length;
    
    final successRate = aiEvents > 0 ? (successfulPregnancies / aiEvents * 100) : 0;
          ...
          children: [
            ...
            Row(
              children: [
                ...
                _buildAnalyticsCard(
                  title: "Avg Interval",
                  value: avgCalvingInterval > 0 ? "${avgCalvingInterval.toStringAsFixed(0)} days" : "N/A",
                  color: Colors.purple,
                ),
                const SizedBox(width: 8),
                _buildAnalyticsCard(
  ...
  double _calculateAverageCalvingInterval(List<Map<String, dynamic>> records) {
    final calvingEvents = records.where((r) => r['eventType'] == 'Calving').toList();
    if (calvingEvents.length < 2) return 0;
    
    // Sort calving events by date
    calvingEvents.sort((a, b) => (a['date'] as Timestamp).compareTo(b['date'] as Timestamp));
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text("Detailed analytics with charts coming soon!")),
              );
            },
                              ...
                              children: [
                                  ...
                                  builder: (context, snapshot) {
                                    ...
                                    return DropdownButtonFormField<String>(
                                      ...
                                      items: animals.map<DropdownMenuItem<String>>((animal) {
                                        ...
                                        final gender = animal["gender"] as String? ?? 'Unknown';
                                      ...
                                      onChanged: (String? val) {
                                        setState(() {
                                          ...
                                          if (val != null) {
                                            ...
                                            _selectedAnimalName = selectedAnimal["name"] as String? ?? "Unknown";
                                          }
                                        });
                                      },
                                      validator: (value) =>
                                    ...
                                    children: [
                                      DropdownButtonFormField<String>(
                                        decoration: const InputDecoration(
                                          ...
                                          hintText: "Select outcome (can be updated later)",
                                        ),
                                        value: _selectedOutcome,
                                        items: _outcomeTypes.map((String outcome) {
                                          return DropdownMenuItem<String>(
                                            value: outcome,
                                            child: Text(
                                              outcome,
                                ...
                                TextFormField(
                                  ...
                                  decoration: const InputDecoration(
                                    ...
                                    hintText: "Additional observations, vet notes, etc...",
                                  ),
                                  maxLines: 3,
                                ),
                                const SizedBox(height: 20),
                    ...
                    builder: (context, snapshot) {
                      ...
                      if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                        return Card(
                          child: Padding(
                            ...
                            child: Column(
                              children: [
                                Icon(Icons.pregnant_woman, size: 64, color: Colors.grey[300]),
                                const SizedBox(height: 16),
                                const Text(
                      ...
                      return ListView(
                        ...
                        children: snapshot.data!.docs.map((doc) {
                          ...
                          final animalName = record["animalName"] ?? "Unknown Animal";
                          final eventType = record["eventType"] ?? "Unknown Event";
                          ...
                          } else if (eventType.toLowerCase().contains('pregnant')) {
                            eventColor = Colors.green;
                            eventIcon = Icons.pregnant_woman;
                          } else if (eventType.toLowerCase().contains('calving')) {
                            eventColor = Colors.blue;
                            eventIcon = Icons.celebration;
                          } else if (eventType.toLowerCase().contains('ai') || 
                                     eventType.toLowerCase().contains('insemination')) {
                            ...
                            child: ListTile(
                                    ...
                                    children: [
                                      ...
                                      if (outcome != 'Pending')
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            color: outcome == 'Pregnancy Confirmed' 
                              ...
                              trailing: PopupMenuButton<String>(
                                onSelected: (value) {
                                  ...
                                  } else if (value == "update_outcome" && eventType == 'Insemination (AI)') {
