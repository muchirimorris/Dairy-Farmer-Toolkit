...
class _FeedOptimizationScreenState extends State<FeedOptimizationScreen> {
  ...
  void initState() {
    ...
    if (user != null) {
      _feedRepo.syncFeed(user.id);
      _feedStream = _feedRepo
          .getFeedInventoryStream(user.id)
          .asBroadcastStream();
    }
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _showAddFeedDialog(context, user?.id),
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (deduct > 0 && deduct <= item.quantity) {
                ...
                await _feedRepo.updateFeedItem(updatedItem);
                if (context.mounted) Navigator.pop(context);
              }
            },
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              await _feedRepo.deleteFeedItem(item.id);
              if (context.mounted) Navigator.pop(context);
            },
