...
class _HealthScreenState extends State<HealthScreen> {
  ...
  void initState() {
    ...
    if (user != null) {
      _healthRepo.syncHealthRecords(user.id);
      _animalRepo.syncAnimals(user.id);

      _healthStream = _healthRepo
          .getHealthRecordsStream(user.id)
          .asBroadcastStream();
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _showAddRecordDialog(context, user?.id),
          ),
        ],
      ),
      body: user == null
          ? const Center(child: Text("Please login to manage health records."))
          : _buildRecordsList(user.id),
    );
  }
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              _healthRepo.deleteHealthRecord(recordId);
              if (context.mounted) Navigator.pop(context);
            },
